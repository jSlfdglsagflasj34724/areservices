...
<div class="p-8">
  <div class="card p-8">
        <form method="POST" action="{{url('addCustomer')}}" enctype="multipart/form-data"> 
      {{ csrf_field() }}
      ...
      <div class="form-group mb-4">
        <input type="password" class="@error('password') is-invalid @enderror w-full  p-3 text-sm font-medium border rounded border-gray-400 placeholder:text-finlendia-base focus:outline-none focus:ring-1 focus:ring-gray-800 focus:border-transparent invalid" id="password" name="password" placeholder="password" value="{{old('password')}}" required="required">
      @error('password')
        <div style='color:#ff0000'; class="alert alert-danger">{{ $message }}</div>
      @enderror  
      </div>
